...
internal sealed class EdgePermutationCoordinate : ICoordinate
{
    ...
    public EdgePermutationCoordinate(ushort coordinate)
    {
    ...
    {
        return new EdgePermutationCoordinate(value);
    }
    ...
    {
        ushort coordinate = 0;
        // we only use edges in the U and D layers to calculate this coordinate
        for (var i = 1; i <= DBIndex; i++)
        {
            var count = 0;
            for (var j = 0; j < i; j++)
            {
    ...
    {
        Span<int> counts = stackalloc int[DBIndex + 1];
        ...
        _edges[FLIndex] = FLIndex;
        _edges[BLIndex] = BLIndex;
        _edges[BRIndex] = BRIndex;
    }
}